 ...
 */
public class CanonizerModalTransposeModel {
    ...
    public  CanonizerModalTransposeModel() {
            resetParams();
            ...
            Pattern music_output = CombineFragments(InputParameters.tempo_bpm, number_of_voices, piece_length);         
...
//                        }
    ...
    public static ArrayList<PitchCandidate>  melodicCheck(ArrayList<PitchCandidate> pitch_candidates, ModeModule my_mode_module, MelodicVoice alter_me, 
        ...
        Integer mode_transpose = InputParameters.transpose_interval;
        for (PitchCandidate myPC : pitch_candidates){
            ...
            if (voice_pitch_count > 0) {
                ...
                double thresh = 0;
		Double threshornull = my_mode_module.getMelodicMotionProbability(cand_pitch, previous_melody_pitch, key_transpose, mode_transpose);
		if (threshornull == null){
                    ...
                    if (threshornull < .05) myPC.decrementRank(Decrements.improbable_melodic_motion);
                    thresh = threshornull;
                    //DEBUG
    ...
    public static ArrayList<PitchCandidate> PitchCandidateVetting (ArrayList<PitchCandidate> pitch_candidates, ModeModule my_mode_module, boolean prog_built, boolean is_accent, int pitch_center) {
        ...
        for (PitchCandidate myPC : pitch_candidates){
            ...
            if (cand_pitch%12 != my_mode_module.getTonic() ){
                //DEBUG
    ...
    public Pattern CombineFragments(Integer tempo_bpm, Integer number_of_voices, Integer piece_length) {
        //Now combine fragments into canon voices for the final pattern - break this into its own method
    ...
    public ArrayList<MelodicVoice> PerformTranspositions(Integer number_of_voices, MelodicVoice nextFragment, ModeModule my_mode_module, int [] transpose_interval_array) {
        ...
        for (int v = 0; v < number_of_voices -1; v++) {
          ...
          //}
            ...
            ArrayList<MelodicNote> t_return_list = new ArrayList();
            ArrayList<MelodicNote> frag_note_list = nextFragment.getNoteArray();
            if (v == 0) fragment_transpose_interval = 0;
            else fragment_transpose_interval = transpose_interval_array[v -1];
            for (MelodicNote mynote : frag_note_list) {
                MelodicNote newnote = new MelodicNote();
                newnote.setAccent(mynote.getAccent());
                newnote.setDuration(mynote.getDuration());
                Integer mynote_pitchclass = mynote.getPitch()%12;
                Integer mynote_transposed_pitchclass = my_mode_module.ModeTranspose(mynote_pitchclass, fragment_transpose_interval);
                Integer pitchdiff = mynote_transposed_pitchclass - mynote_pitchclass;
                if (fragment_transpose_interval > 12) pitchdiff += (fragment_transpose_interval - fragment_transpose_interval%12);
                newnote.setPitch(mynote.getPitch() + pitchdiff);
                System.out.println("old note " + mynote.getPitch() + " to new note" + newnote.getPitch());
