    ...
    public  Model_1() {
            ...
            for (byte i = 0; i < built_voices.size(); i++){
                ...
                for (MelodicNote final_note : final_note_array) {
                    ...
                    voice_pattern.addElement(jf_note);
                } // end create a jfugue musicstring from the built voice loop
                
                //Gorecki canon section -
                int repeats;
                int cutoff;

                if (i %2 == 0) cutoff = number_of_voices/2;
                else cutoff = (number_of_voices + 1)/2;
                
                if (i < cutoff) {
                    repeats = number_of_voices;
                }
                else {
                    repeats = number_of_voices - (i - cutoff)*2;
                }
                System.out.println("padding rests " + padding_rests);
                for (int a = 0; a < padding_rests; a++) {
                    music_output.addElement(jf_voice);
                    music_output.add("Rw Rw Rw Rw");
                }
                padding_rests++;
                ...
                for (int b = 0; b < built_voice_queues.size(); b++){
                    ...
                    do {
                        ...
                        if (!skip_me){
                                        ...
                                        //HERE ALSO EVALUATE PITCH CANDIDATES AGAINST NEW harmonicChecksInv
    ...
    public static ArrayList<PitchCandidate>  melodicCheck(ArrayList<PitchCandidate> pitch_candidates, ModeModule my_mode_module, MelodicVoice alter_me, 
        ...
        for (PitchCandidate myPC : pitch_candidates){
            ...
            if (voice_pitch_count > 0) {
                ...
                double thresh = 0;
				Double threshornull = my_mode_module.getMelodicMotionProbability(cand_pitch, previous_cp_pitch, key_transpose, 0);
